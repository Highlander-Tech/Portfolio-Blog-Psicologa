...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const blog = await prismic.query([Prismic.Predicates.at('document.type', 'blog')], {
    pageSize: 1,
  })

  const blogPosts = await prismic.query([Prismic.Predicates.at('document.type', 'post')], {
    pageSize: 100,
  })
...
export default function TemplateBlog(props: any) {
  ...
  useEffect(() => {
    setCards(
      artigos.slice(0, index).map((artigo: any) => {
        return (
          <Card
            title={artigo.title}
  ...
  return (
    <Box bgImage="59.png" bgRepeat="no-repeat" w="full" h="1500px" borderBottom="1px solid #474544;">
      <Flex w="1210px" mx="auto">
        <Box w="860px">
          <Box borderRight="1px solid #474544" pt="32px" h="1499px" overflow={overflow} overflowX="hidden">
            {Cards}
            <Box w="255px" mx="auto">
              <Button
                onClick={handleClick}
        ...
        <Box ml="50px">
          <Sobre
            ...
            photo={props.photo}
          />
          <Box w="300px" minH="250px" border="1px solid #E9CBBB" mt="40px" pl="10px" pt="10px">
            <Text
              fontFamily="Roboto"
              color="#474544"
              fontSize="18px"
